...
if st.session_state.page == "saisie":
        ...
        if img_barcode:
            with st.spinner("🔍 Analyse du code-barres..."):
                ...
                if codes:
                    ...
                    st.session_state.scan_counter += 1  # Incrémenter le compteur pour forcer la mise à jour
                    ...
                    <div class="auto-filled">
                        <strong>✨ Saisie automatique :</strong> Le code a été automatiquement inséré dans le champ "Nom de la pièce" ci-dessous.
                    </div>
                    """, unsafe_allow_html=True)
                    
                    # Forcer le rechargement pour mettre à jour le champ
                    st.rerun()
                else:
    ...
    if st.session_state.scan_effectue:
        if st.button("🔄 Nouveau scan", use_container_width=True):
            st.session_state.scan_effectue = False
            st.session_state.code_detecte = None
            st.session_state.scan_counter += 1  # Incrémenter pour forcer la mise à jour
            st.rerun()
    ...
    # Utiliser une clé dynamique qui change à chaque scan
    input_key = f"nom_piece_input_{st.session_state.scan_counter}"
    ...
    with col_nom:
        ...
        if st.session_state.scan_effectue and st.session_state.code_detecte:
            if nom_piece == st.session_state.code_detecte:
