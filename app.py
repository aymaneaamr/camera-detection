...
class GestionnairePieces:
    def __init__(self):
        ...
        self.pieces = {}  # Dictionnaire {nom_piece: {"libelle": "", "photos": [], "emplacement": ""}}
    ...
    def creer_nouvelle_piece(self, nom_piece, libelle="", emplacement=""):
        """Crée une nouvelle pièce dans l'inventaire avec son libellé et emplacement"""
    ...
    def get_libelle_piece(self, nom_piece):
        """Retourne le libellé d'une pièce"""
        if nom_piece in self.pieces:
            return self.pieces[nom_piece].get('libelle', '')
        return ''
    ...
    def get_tous_libelles(self):
        """Retourne un dictionnaire avec tous les libellés par pièce"""
        return {nom: self.get_libelle_piece(nom) for nom in self.pieces}
    ...
    def generer_excel(self):
        ...
        headers = ["Nom de la pièce", "Libellé", "Emplacement", "Quantité totale", "Nombre de photos", "Dernière mise à jour"]
        ...
        detail_headers = ["Pièce", "Libellé", "Emplacement", "Photo #", "Date", "Nombre de pièces"]
...
4. **Ajouter** plusieurs photos pour cette pièce
5. **Changer** de pièce et répéter
...
with st.sidebar:
    ...
    if gestionnaire.pieces:
        # Afficher toutes les pièces avec leurs totaux, libellés et emplacements
        for nom_piece in gestionnaire.pieces.keys():
            total = gestionnaire.get_total_piece(nom_piece)
            libelle = gestionnaire.get_libelle_piece(nom_piece)
...
if st.session_state.page == "saisie":
    ...
    # Formulaire de création de pièce avec libellé et emplacement optionnels
    ...
    with col1:
        if st.button("✅ Créer la pièce", use_container_width=True):
            if nom_piece:
                if gestionnaire.creer_nouvelle_piece(nom_piece, libelle, emplacement):
    ...
    with col_h1:
        ...
        if libelle:
            st.markdown(f"<span class='label-badge'>🏷️ {libelle}</span>", unsafe_allow_html=True)
        if emplacement:
...
with col_f4:
    ...
    st.caption(f"🏷️ Libellés: {libelles_renseignes}/{len(gestionnaire.pieces)} | 📍 Empl.: {emplacements_renseignes}/{len(gestionnaire.pieces)}")
