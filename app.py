...
class GestionnairePieces:
    def __init__(self):
        ...
        self.pieces = {}  # Dictionnaire {article: {"photos": [], "emplacement": "", "libelle": ""}}
    ...
    def creer_nouvel_article(self, article, emplacement="", libelle=""):
        """Crée un nouvel article dans l'inventaire avec son emplacement et libellé"""
        if article and article not in self.pieces:
            self.pieces[article] = {
                'photos': [],
    ...
    def ajouter_photo_article(self, article, frame_original, frame_analyse, nb_pieces):
        """Ajoute une photo analysée à un article existant"""
        if article in self.pieces:
            ...
            photo_data = {
                ...
                'id': len(self.pieces[article]['photos'])
            }
            
            self.pieces[article]['photos'].append(photo_data)
            return True
        return False
    
    def get_total_article(self, article):
        """Retourne le total de pièces pour un article donné"""
        if article in self.pieces:
            return sum(photo['nb_pieces'] for photo in self.pieces[article]['photos'])
        return 0
    
    def get_photos_article(self, article):
        """Retourne toutes les photos d'un article"""
        if article in self.pieces:
            return self.pieces[article]['photos']
        return []
    
    def get_emplacement_article(self, article):
        """Retourne l'emplacement d'un article"""
        if article in self.pieces:
            return self.pieces[article].get('emplacement', '')
        return ''
    
    def get_libelle_article(self, article):
        """Retourne le libellé d'un article"""
        if article in self.pieces:
            return self.pieces[article].get('libelle', '')
        return ''
    
    def supprimer_photo(self, article, photo_id):
        """Supprime une photo d'un article"""
        if article in self.pieces and 0 <= photo_id < len(self.pieces[article]['photos']):
            del self.pieces[article]['photos'][photo_id]
            # Réindexer les IDs
            for i, photo in enumerate(self.pieces[article]['photos']):
                photo['id'] = i
            return True
        return False
    ...
    def generer_excel(self):
        ...
        # En-têtes (ajout des colonnes Emplacement et Libellé)
        headers = ["Article", "Emplacement", "Libellé", "Quantité totale", "Nombre de photos", "Dernière mise à jour"]
        ...
        # Données du résumé
        row = 2
        for article, data in self.pieces.items():
            ...
            sheet_resume.cell(row=row, column=1).value = article
            sheet_resume.cell(row=row, column=2).value = emplacement
            sheet_resume.cell(row=row, column=3).value = libelle
        ...
        sheet_resume.column_dimensions['C'].width = 30
        ...
        # En-têtes détail (ajout des colonnes Emplacement et Libellé)
        detail_headers = ["Article", "Emplacement", "Libellé", "Photo #", "Date", "Nombre de pièces"]
        ...
        for article, data in self.pieces.items():
            ...
            for i, photo in enumerate(data['photos'], 1):
                sheet_detail.cell(row=row, column=1).value = article
                sheet_detail.cell(row=row, column=2).value = emplacement
                sheet_detail.cell(row=row, column=3).value = libelle
        ...
        sheet_detail.column_dimensions['C'].width = 30
...
st.title("📦 Gestionnaire d'Inventaire avec Scan Code-Barres")
st.markdown("""
Cette application permet de gérer l'inventaire de plusieurs articles :
1. **Scanner** un code-barres pour identifier automatiquement l'article
2. **Ajouter** un emplacement de stockage (optionnel)
3. **Ajouter** un libellé descriptif (optionnel)
...
with st.sidebar:
    ...
    if gestionnaire.pieces:
        for article in gestionnaire.pieces.keys():
            total = gestionnaire.get_total_article(article)
            emplacement = gestionnaire.get_emplacement_article(article)
            libelle = gestionnaire.get_libelle_article(article)
            ...
            with st.container():
                ...
                with col1:
                    # Afficher l'article avec l'emplacement et le libellé en markdown
                    display_text = f"**📦 {article}**"
                    if libelle:
                        display_text += f"\n📝 `{libelle}`"
                    if emplacement:
                        display_text += f"\n📍 `{emplacement}`"
                    ...
                    # Bouton de sélection
                    if st.button("Sélectionner", key=f"select_{article}", use_container_width=True):
                        st.session_state.article_selectionne = article
...
if st.session_state.page == "saisie":
    ...
    # Formulaire de création d'article avec emplacement et libellé optionnels
    st.markdown("### 📝 Informations de l'article")
    
    # Utiliser une clé dynamique pour le text_input qui change quand le code est détecté
    input_key = f"article_input_{st.session_state.code_detecte or 'manuel'}"
    ...
    # Trois colonnes pour l'article, l'emplacement et le libellé
    col_art, col_emp, col_lib = st.columns([2, 1, 2])
    ...
    with col1:
        if st.button("✅ Créer l'article", use_container_width=True):
            if article:
                if gestionnaire.creer_nouvel_article(article, emplacement, libelle):
                    st.success(f"✅ Article '{article}' créé avec succès!")
                    if emplacement:
    ...
    article = st.session_state.article_selectionne
    photos = gestionnaire.get_photos_article(article)
    total = gestionnaire.get_total_article(article)
    emplacement = gestionnaire.get_emplacement_article(article)
    libelle = gestionnaire.get_libelle_article(article)
    
    # En-tête avec emplacement et libellé
    ...
    with col_h4:
        if emplacement:
            st.metric("Emplacement", emplacement[:10] + "..." if len(emplacement) > 10 else emplacement)
    ...
    with col_o3:
        if st.button("🗑️ Supprimer cet article", use_container_width=True, type="primary"):
            if gestionnaire.supprimer_article(article):
                st.success(f"✅ Article '{article}' supprimé")
                st.session_state.page = "saisie"
                st.rerun()
        ...
        if source == "📸 Prendre une photo":
            ...
            if img_file:
                with st.spinner("Analyse..."):
                    ...
                    if gestionnaire.ajouter_photo_article(article, frame, resultat, nb_pieces):
    ...
    if photos:
        ...
        for i, photo in enumerate(photos_affichees):
            with cols[i % 3]:
                ...
                with col_b1:
                    if st.button("🔍 Voir", key=f"view_{article}_{i}"):
                        st.session_state.photo_selectionnee = photo['id']
                        st.session_state.page = "photo_detail"
                        st.rerun()
                with col_b2:
                    if st.button("🗑️", key=f"del_{article}_{i}"):
                        if gestionnaire.supprimer_photo(article, photo['id']):
                            st.rerun()
    ...
    if 0 <= photo_id < len(photos):
        ...
        st.header(f"🔍 Détail de la photo - {article}")
        ...
        with col_b2:
            if st.button("🗑️ Supprimer cette photo", use_container_width=True, type="primary"):
                if gestionnaire.supprimer_photo(article, photo_id):
                    st.session_state.page = "details"
                    st.session_state.photo_selectionnee = None
                    st.rerun()
    else:
...
with col_f3:
    st.caption(f"📊 Articles: {len(gestionnaire.pieces)}")
with col_f4:
    ...
    st.caption(f"📍 {emplacements_renseignes}/{len(gestionnaire.pieces)} | 📝 {libelles_renseignes}/{len(gestionnaire.pieces)}")
